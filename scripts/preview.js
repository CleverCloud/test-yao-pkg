...
//   CC_CLEVER_TOOLS_PREVIEWS_CELLAR_BUCKET      Preview storage bucket (required)
...
runCommand(async () => {
  ...
  const previewName = getVersion(rawArg ?? (await getCurrentBranch()));
  const os = getOs();
  ...
  switch (command) {
    ...
    case 'update': {
      return updatePreviews(rawArg ?? '*', os);
    }
    case 'delete': {
      await deleteRemotePreview(previewName);
      return deleteLocalPreviews(previewName);
    }
    case 'pr-comment': {
...
async function buildPreview(previewName, os) {
  ...
  await mkdir(binaryParts.directory, { recursive: true });
...
/**
 * Creates a regex pattern from a glob-like filter string.
 * Converts * to .* and ? to . for pattern matching.
 * @param {string} filter - The glob-like filter pattern.
...
async function updatePreviews(filter, os) {
  ...
  const filteredRemotePreviews = Object.entries(remoteManifest.previews)
    .filter(([previewName]) => regex.test(previewName))
  ...
  await Promise.all(
    filteredRemotePreviews.map(([previewName, preview, state], rowIdex) => {
      switch (state) {
        case 'no-build':
          return table.updateData(rowIdex, lastColumnIndex, 'No build for OS', 'gray');
        case 'downloading':
        case 'updating':
  ...
  await mkdir(LOCAL_PREVIEW_DIR, { recursive: true });
...
async function downloadPreview(previewName, preview, os, updateState) {
  ...
  try {
    ...
    await mkdir(tarPath.directory, { recursive: true });
    await writeFile(`${tarPath.directory}/${tarPath.filename}`, downloadBuffer);
...
async function installBinary(previewName, os, quiet) {
  ...
  await mkdir(destinationParts.directory, { recursive: true });
 ...
 * @returns {Manifest} The local manifest object.
 * @throws {Error} When the manifest file cannot be read or parsed.
 */
function getLocalManifest() {
  return readJson(LOCAL_MANIFEST_PATH) ?? createDefaultManifest();
}
